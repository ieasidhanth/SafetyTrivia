...
namespace Quizzed
{
    public partial class Quiz_RC : Form
    {
        //private DSL dslObj;
        ...
        private List<int> failed_testIds;
        ...
        public Quiz_RC(DataTable dt, DataSet DataBank)
        {
            ...
            pos_category = Convert.ToString(row["Category"]);
            DOB = Convert.ToString(row["DOB"]);
            lblPositionPlcHldr.Text = pos_category;
            failed_testIds = new List<int>();
            string[] remaining_test = Convert.ToString(row["Remaining_test"]).Split(';');
            foreach(string test in remaining_test)
            {
                string temp = test.Split(':')[0];
                if(temp!="")
                    failed_testIds.Add(Convert.ToInt32(temp.Split(' ')[1].Trim()));
            }
            QuizNo = failed_testIds[0];
            count_failed_quiz = 0;
            currentQuestionIndex = 0;
            currentQuestionAnswered = false;
            tabAttemptCount = 1;
            lblName.Text = Convert.ToString(row["First_name"]+"_"+row["Last_name"]);
            lblJobName.Text = Convert.ToString(row["Job"]);
            name = Convert.ToString(row["First_name"] + "_" + row["Last_name"]);
            jobName = Convert.ToString(row["Job"]);
            ...
            quizResults.Columns.Add("Category", typeof(string));
        ...
        private void Quiz_Load(object sender, EventArgs e)
        {
            ...
            qBank = dslObj.getallQuestions(failed_testIds,Convert.ToString(candidateTable.Rows[0]["Category"]));
            if(qBank.Rows.Count!=0)
            {
                ...
                foreach (DataRow video in VideoLinks.Rows)
                {
                    ...
                    lbl.Text = Convert.ToString(video["Link_text"]);
                ...
                foreach (int quizId in failed_testIds)
                {
                    ...
                    qpanel.Controls.Add(new Label() { Name = "pnl_tab_Quiz_Questions_lbl_QHeading" + Convert.ToString(i + 1), Text = "QHeading", Location = new System.Drawing.Point(7, 16), Margin = new Padding(3, 0, 3, 0), Font = new System.Drawing.Font("Verdana", 28, FontStyle.Bold, GraphicsUnit.Pixel), AutoSize = true });
                    ...
                    foreach (Control ctrl in qpanel.Controls)
                    {
                        if (ctrl.Name == "pnl_tab_Quiz_Questions_lbl_Q" + Convert.ToString(i + 1))
                        {
                            ComponentResourceManager resources = new ComponentResourceManager(typeof(Quiz_RC));
                            resources.ApplyResources(ctrl, ctrl.Name, new CultureInfo("es"));

                        }

                    }
                    tabarray[i].Controls.Add(qpanel);
                    tabarray[i].Controls.Add(new Panel() { Name = "pnl_tab_Quiz_Answers" + Convert.ToString(i + 1), Location = new System.Drawing.Point(10, 150), Width = 1468, Height = 300, BackColor = Color.FromArgb(224, 237, 241) });
                    ...
                    tabarray[i].Controls.Add(new Panel() { Name = "pnl_tab_Quiz_Navigation" + Convert.ToString(i + 1), Location = new System.Drawing.Point(10, 300), BackColor = Color.Bisque, Width = 750, Height = 300 });
            ...
            {
                MessageBox.Show(" No quiz found for selected category " + candidateTable.Rows[0]["Category"]);
               // this.Close();
        ...
        private void proceed_Click(object sender, EventArgs e)
        {
            ...
            if(!passStatus)
            {
                DataRow crow = candidateTable.Rows[0];
                if(Convert.ToString(crow["Hire_Status"])=="Conditional Hire")
                {
                    addTestDetails(name.Split('_')[0], name.Split('_')[1], tbcontrolQuizzes.SelectedTab.Name.Split(' ')[1], Convert.ToString(dt.Rows[0]["QuizName"]), "Fail", "", pos_category);
                    ...
                    store_local_history(quizResults, jobName, name, pos_category, DOB);
                    try
                    {
                        Upload_RC formnew = new Upload_RC(quizResults, jobName, name, candidateID, "Fail", DOB,dBank);
                        this.Hide();
                        formnew.Show();
                        //MessageBox.Show("Status updated, Kindly check Sharepoint");
                        ////Application.Exit();
                        //new Login_offline(dBank);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (count_failed_quiz <= max_fail_quiz)
                        {
                            ...
                            //Application.Exit();
                            DataRow row = dt.Rows[0];
                            addTestDetails(name.Split('_')[0], name.Split('_')[1], tbcontrolQuizzes.SelectedTab.Name.Split(' ')[1], Convert.ToString(row["QuizName"]), "Fail", "",pos_category);
                            ...
                            {
                                store_local_history(quizResults, jobName, name, pos_category, DOB);
                                MessageBox.Show("End of Orientation.Kindly Contact Site Safety Manager");
                                this.Hide();
                                try
                                {
                                    Upload_RC formnew = new Upload_RC(quizResults, jobName, name, candidateID, "Pass", DOB,dBank);
                                    formnew.Show();
                                    //this.Hide();
                                    //Thread.Sleep(2000);
                                    //MessageBox.Show("Upload Complete.Check Sharepoint Site\n\n\nContact the administrator. Click Ok to continue", "Continue");
                                    //formnew.Hide();
                                    //new Login_offline(dBank).Show();
            ...
            else
            {
                ...
                DataRow row = dt.Rows[0];
                addTestDetails(name.Split('_')[0],name.Split('_')[1],tbcontrolQuizzes.SelectedTab.Name.Split(' ')[1],Convert.ToString(row["QuizName"]), "Pass",path,pos_category);
                previous_quiz_Comleted = true;
                lockedQuiz.Add(tbcontrolQuizzes.SelectedIndex);
                if (tbcontrolQuizzes.SelectedIndex != (tbcontrolQuizzes.TabCount - 1))
                {
        ...
        public void populateQuestionsTableTab(int quizNumber)
        {
            if(tbcontrolQuizzes.SelectedIndex>=0)
            {
               ...
               // tabResponses.PrimaryKey = keys;
                ...
                tabResponses.Columns.Add("UserCategory", typeof(string));
        ...
        private void checkedChanged(object sender, EventArgs e)
        {
            ...
            if(radiobtn!=null)
            {
                if(radiobtn.Checked)
                {
                    ...
                    response["UserCategory"] = pos_category;
        ...
        private void tbcontrolQuizzes_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (tbcontrolQuizzes.SelectedIndex >= 0)
            {
                string quizId = tbcontrolQuizzes.SelectedTab.Name.Split(' ')[1];

                populateQuestionsTableTab(Convert.ToInt32(quizId));
                displayQuestion(0, tbcontrolQuizzes.SelectedIndex);
                currentQuestionIndex = 0;
            }
        ...
        public void addTestDetails(string fname,string lastName, string quizNumber,string quizname, string status, string filepath,string category)
        {
        ...
        private void Quiz_RC_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        public Boolean store_local_history(DataTable qresults, string jobName, string name, string pos_category_name, string cand_DOB)
        {
            ...
            if (!File.Exists(local_history_path + "\\History.xml"))
            {
                ...
                ReturningCandidate.InnerText = "Yes";
                XmlNode CandidateID = doc.CreateElement("candidateID");
                CandidateID.InnerText = candidateID;
